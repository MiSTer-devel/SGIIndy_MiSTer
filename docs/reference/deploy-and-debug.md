...
```sh
[SEED=n] [BUILD_DATE=YYMMDD] bash scripts/build.sh [--log FILE]
```
...
- **`SEED`.** The HDMI pixel clock's domain, the MiSTer scaler, has sat within
  ...
  seed is the remedy. `sgiindy.qsf` keeps `SEED 1`, the seed the current
  release met timing with.
- **`BUILD_DATE`.** The core's version in the MiSTer menu is the build date,
  which `build.sh` writes into the gitignored `build_id.v` through
  `sys/build_id.tcl` before synthesis, as `--flow compile` would. The date is
  part of the logic, so a build made on another day is a different fit: build
  44's RTL met timing dated 260916 at seed 2 and missed it by up to 0.7 ns on
  the 50 MHz core clock dated 260918 at seeds 2, 3 and 5. Read
  `reports/summary.md` after every build. `BUILD_DATE=YYMMDD` pins the date to
  rebuild an old bitstream exactly; `SEED=1 BUILD_DATE=260918` rebuilds the
  current release.
