...
#
# Usage: [SEED=n] [BUILD_DATE=YYMMDD] bash scripts/build.sh [--log FILE]
set -u
...
# build_id.v is the date the MiSTer menu shows as the core's version: sgiindy.sv
# includes it, and it is gitignored because every compile writes it. Quartus
# writes it only through PRE_FLOW_SCRIPT_FILE (sys/build_id.tcl), which runs
# for `--flow compile` and NOT for the stages this script runs one by one - so
# a fresh clone had no build_id.v and failed synthesis, and an existing
# checkout kept whatever date was last written (build 44 was built on
# 2026-09-18 and says v260916). Run the same script here. BUILD_DATE=YYMMDD
# pins the date instead, which is how an old release is rebuilt bit for bit.
: > "$LOG"
if [ -n "${BUILD_DATE:-}" ]; then
    case "$BUILD_DATE" in
        [0-9][0-9][0-9][0-9][0-9][0-9]) ;;
        *) log "ERROR: BUILD_DATE must be YYMMDD, not '$BUILD_DATE'"; exit 1 ;;
    esac
    printf '`define BUILD_DATE "%s"' "$BUILD_DATE" > build_id.v
else
    "$SH_EXE" -t sys/build_id.tcl compile "$PROJECT_NAME" "$PROJECT_NAME" >> "$LOG" 2>&1 || {
        log "ERROR: sys/build_id.tcl failed"; exit 1; }
fi
log "build date $(sed -n 's/.*"\([0-9]*\)".*/\1/p' build_id.v)"
